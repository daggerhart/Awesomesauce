...
if ( ! function_exists( 'awesomesauce_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function awesomesauce_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'awesomesauce' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'awesomesauce' ),
...
if ( ! function_exists( 'awesomesauce_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function awesomesauce_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && awesomesauce_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'awesomesauce' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'awesomesauce' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'awesomesauce' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'awesomesauce' ), esc_html__( '1 Comment', 'awesomesauce' ), esc_html__( '% Comments', 'awesomesauce' ) );
		echo '</span>';
	}

	edit_post_link( esc_html__( 'Edit', 'awesomesauce' ), '<span class="edit-link">', '</span>' );
}
endif;
